...
void LMesh::CalcNormals(bool useSmoothingGroups)
{
    ...
    else
    {
        ...
        for (i=0; i<m_vertices.size(); i++)
        {
			...
			if ((t>0)&&(temp.x*temp.x+temp.y*temp.y+temp.z*temp.z<0.001))//Added by PVAUT !!!
			{
                temp.x = m_tris[array[i][0]].normal.x;
                temp.y = m_tris[array[i][0]].normal.y;
                temp.z = m_tris[array[i][0]].normal.z;
			}
            m_normals[i] = NormalizeVector(temp);
        }
...
bool L3DS::FindChunk(LChunk &target, const LChunk &parent)
{
    ...
    while (( chunk.id != target.id) && (chunk.end <= parent.end))
    {
        ...
        if (chunk.end >= parent.end)
            break;
		if (chunk.end<chunk.start) break;
        chunk = ReadChunk();
    }
