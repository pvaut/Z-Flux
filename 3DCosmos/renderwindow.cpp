...
void TRenderWindow::UpdateStencil(int InterleavePattern, bool isrightpart)
{
	int resx = renderviewport.lx;
	int resy = renderviewport.ly;


	glMatrixMode(GL_PROJECTION);
	glLoadIdentity();
	glOrtho(0.0, (double) resx, 0.0, (double) resy, -1.0, 1.0);
	...
	if (InterleavePattern == 1) {
		if (!isrightpart)
			glPolygonStipple(patternVEven);
		else
			glPolygonStipple(patternVOdd);
	}
	if (InterleavePattern == 2) {
		if (!isrightpart)
			glPolygonStipple(patternVOdd);
		else
			glPolygonStipple(patternVEven);
	}
	if (InterleavePattern == 3) {
		if (!isrightpart)
			glPolygonStipple(patternHEven);
		else
			glPolygonStipple(patternHOdd);
	}
	if (InterleavePattern == 4) {
		if (!isrightpart)
			glPolygonStipple(patternHOdd);
		else
			glPolygonStipple(patternHEven);
...
{
	int InterleavePattern = disp->G_param(_qstr("InterleavePattern"))->content.G_content_scalar()->G_intval();
	if ( (!viewport->RenderToTexture) && (InterleavePattern>0) )
		throw QError(_qstr("Interleave pattern must be used in conjuction with render to texture mode"));
	...
	if (viewport->RenderToTexture)
	{
		...
		if (InterleavePattern>0)
			UpdateStencil(InterleavePattern, isrightpart);
