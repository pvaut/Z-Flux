...
void T3DObjectMesh::generatevertexproperty(TSC_functor *functor, int vertexproperty)
{
	...
	try{
		...
		for (int i=0; i<mesh.vertices.G_count(); i++)
		{
			...
			if (vertexproperty==VertexPropertyRed) mesh.vertices_col[i].r=retval0.G_content_scalar()->G_val();
			if (vertexproperty==VertexPropertyGreen) mesh.vertices_col[i].g=retval0.G_content_scalar()->G_val();
			if (vertexproperty==VertexPropertyBlue) mesh.vertices_col[i].b=retval0.G_content_scalar()->G_val();
			if (vertexproperty==VertexPropertyTransp) mesh.vertices_col[i].a=retval0.G_content_scalar()->G_val();
...
void T3DObjectMesh::CreateExtrudedPolygon(T2DContourset *contourset0, double height, bool closed, int layercount)
{
	...
	//create augmented contour set with all corner points explicitely splitted up
	T2DContourset contourset2;
	contourset2=(*contourset0);
	contourset2.splitautocornerpoints();

	//calculate total number of points in a single layer
	totlayerpointcount=0;
	for (int contournr=0; contournr<contourset2.G_contourcount(); contournr++)
	{
		T2DContour *contour=contourset2.G_contour(contournr);
...
///////////////////////////////////////////////////////////////////////////////////
FUNCTION(func_objectmesh_createextrudedshape,CreateExtrudedShape)
{
	setmemberfunction(SC_valname_meshobject);
	addvar(_qstr("contour"),SC_valname_2dcontourset);
	addvar(_qstr("height"),SC_valname_scalar);
	addvar(_qstr("closed"),SC_valname_boolean);
	addvar(_qstr("layercount"),SC_valname_scalar);
}
void execute_implement(TSC_funcarglist *arglist, TSC_value *retval, TSC_value *assigntoval, TSC_value *owner)
{
	bool closed=true;
